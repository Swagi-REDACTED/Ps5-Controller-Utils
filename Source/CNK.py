...
# --- Methods for pressing/releasing keys, unified wrappers ---
    ...
    def _press_key_pydirectinput(self, key_str):
        ...
        if not key_str:
            return
        try:
            mapped = PYDIRECTINPUT_KEY_MAP.get(key_str, key_str.lower())
            print(f"Debug: Pressing '{key_str}' as '{mapped}' with pydirectinput")
    ...
    def _release_key_pydirectinput(self, key_str):
        ...
        try:
            mapped = PYDIRECTINPUT_KEY_MAP.get(key_str, key_str.lower())
            print(f"Debug: Releasing '{key_str}' as '{mapped}' with pydirectinput")
    ...
    def draw_analog_sticks(self, draw):
        """
        draw: PIL.ImageDraw.Draw instance (on an RGBA image)
        This remains mostly the same but uses the safer draw_stick_indicator below.
        """
        active_settings = self.get_active_settings()
        ...
        for stick_name, base_pos in stick_positions.items():
            ...
            x_offset_norm = (x_val - 127.5) / 127.5
            y_offset_norm = (y_val - 127.5) / 127.5
    ...
    def draw_stick_indicator(self, draw, base_pos, x_val, y_val, inner_dz=0.0, outer_dz=1.0, debug=False):
        """
        Robust PIL-based stick indicator that won't disappear when the image is small.
        - draw: PIL.ImageDraw.Draw instance
        - base_pos: (x,y) in the 1200x800 design coordinates (will scale to current image)
        - x_val,y_val: normalized [-1..1]
        - inner_dz, outer_dz: 0..1 values (outer_dz >= inner_dz)
        """
        # defensively read image size
        img_w = max(1, getattr(self.working_tester_image, "width", None) or self.working_tester_image.size[0])
        img_h = max(1, getattr(self.working_tester_image, "height", None) or self.working_tester_image.size[1])

        # scale relative to your design canvas (1200x800)
        scale_x, scale_y = img_w / 1200.0, img_h / 800.0
        uniform_scale = max(0.05, min(scale_x, scale_y))  # enforce a sensible minimum scale

        # scaled base coordinates
        base_x = int(base_pos[0] * scale_x)
        base_y = int(base_pos[1] * scale_y)

        # compute ring radius (enforce minimum)
        ring_radius = max(6, int(round(60 * uniform_scale)))

        # stroke widths must be at least 1
        outline_w = max(1, int(round(2 * uniform_scale)))
        dz_outline_w = max(1, int(round(1.5 * uniform_scale)))
        line_w = max(1, int(round(2 * uniform_scale)))
        dot_radius = max(3, int(round(8 * uniform_scale)))

        # ensure deadzones are sane
        inner_dz = float(max(0.0, min(0.99, inner_dz)))
        outer_dz = float(max(inner_dz + 0.001, min(1.0, outer_dz)))

        # pixel radii for deadzones (ensure minimum separation)
        inner_px = max(1.0, inner_dz * ring_radius)
        outer_px = max(inner_px + 1.0, outer_dz * ring_radius)
        if outer_px > ring_radius:
            outer_px = ring_radius
        if outer_px - inner_px < 1.0:
            # maintain at least 1px gap
            inner_px = max(0.0, ring_radius - 2.0)
            outer_px = ring_radius

        if debug:
            print(f"DBG: img {img_w}x{img_h} scale {uniform_scale:.3f} ring {ring_radius} inner_px {inner_px:.1f} outer_px {outer_px:.1f}")

        # If the ring can't be drawn sensibly, draw a guaranteed-visible fallback (single filled circle)
        if ring_radius < 6 or min(img_w, img_h) < 16:
            fallback_radius = max(2, dot_radius)
            draw.ellipse(
                [base_x - fallback_radius, base_y - fallback_radius, base_x + fallback_radius, base_y + fallback_radius],
                fill=(0, 122, 204, 255)
            )
            return

        # ensure bbox coords are integers
        ring_bbox = [int(base_x - ring_radius), int(base_y - ring_radius), int(base_x + ring_radius), int(base_y + ring_radius)]
        inner_bbox = [int(base_x - inner_px), int(base_y - inner_px), int(base_x + inner_px), int(base_y + inner_px)]
        outer_bbox = [int(base_x - outer_px), int(base_y - outer_px), int(base_x + outer_px), int(base_y + outer_px)]

        # dashed-like deadzone rings: PIL doesn't support "dash" for ellipses, so we draw normally with a visible stroke
        draw.ellipse(inner_bbox, outline=(0, 255, 255, 220), width=dz_outline_w)
        draw.ellipse(outer_bbox, outline=(120, 70, 215, 220), width=dz_outline_w)

        # compute dot location, but keep it inside ring bounds accounting for dot radius and stroke
        vec = math.hypot(x_val, y_val)
        # clamp normalized vector to [-1,1]
        if vec > 1.0:
            x_val /= vec
            y_val /= vec
            vec = 1.0

        # if within inner deadzone, dot sits at center; else compute radial position between inner_px and outer_px
        dot_x = base_x
        dot_y = base_y
        if vec > inner_dz:
            capped_vec = min(vec, outer_dz)
            dz_range = outer_dz - inner_dz if outer_dz > inner_dz else 1.0
            scaled = max(0.0, min(1.0, (capped_vec - inner_dz) / dz_range))
            ux = x_val / vec if vec > 0 else 0.0
            uy = y_val / vec if vec > 0 else 0.0
            final_r = inner_px + scaled * (outer_px - inner_px)
            # keep the dot inside the ring by subtracting dot radius + outline
            allowed_r = max(0.0, ring_radius - (dot_radius + outline_w))
            final_r = min(final_r, allowed_r)
            dot_x = base_x + ux * final_r
            dot_y = base_y + uy * final_r

        # clamp dot coordinates to image bounds
        dot_x = max(dot_radius, min(img_w - dot_radius - 1, int(round(dot_x))))
        dot_y = max(dot_radius, min(img_h - dot_radius - 1, int(round(dot_y))))

        # color: cyan when outside inner deadzone, red when inside
        dot_color = (0, 255, 255, 220) if vec > inner_dz else (255, 0, 0, 220)

        # draw line (only if it has visible length)
        if math.hypot(dot_x - base_x, dot_y - base_y) > 1.0:
            draw.line([base_x, base_y, dot_x, dot_y], fill=dot_color, width=line_w)

        # draw final dot (fill + white outline)
        draw.ellipse(
            [dot_x - dot_radius, dot_y - dot_radius, dot_x + dot_radius, dot_y + dot_radius],
            fill=dot_color, outline=(255, 255, 255, 255), width=max(1, outline_w // 2)
        )
