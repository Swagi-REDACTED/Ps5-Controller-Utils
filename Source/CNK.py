...
# pynput is used for robust, low-level keyboard and mouse control.
# On Windows, it uses the Win32 SendInput function, which is a reliable way
# to simulate input for many applications and games.
#
# This script uses `pynput` (SendInput), which is a higher-level API. While
# effective, some aggressive anti-cheat systems can detect and block API-based
# input.
#
# This script uses `pynput` (SendInput), which is a higher-level API. While
# effective, some aggressive anti-cheat systems can detect and block API-based
...
# input.
#
# The enhancements in this file focus on making the behavior and style of the input
...
class UtilityApp:
    ...
    def _ensure_default_settings(self, settings_dict):
        ...
        settings_dict.setdefault('adaptive_game_mode', False)  # Add this new line
        settings_dict.setdefault('dualsense_sensitivity', 15.0)
        settings_dict.setdefault('dualsense_inner_deadzone', 0.08)
        settings_dict.setdefault('dualsense_outer_deadzone', 0.95)
        ...
        settings_dict.setdefault('game_exponent', 2.4) # CHANGED: More responsive default
        settings_dict.setdefault('game_acceleration_enabled', False)
        settings_dict.setdefault('game_acceleration_rate', 0.1)
        settings_dict.setdefault('game_sensitivity_multiplier', 1.5) # NEW: Game mode speed boost
...
# --- Methods for pressing/releasing keys, unified wrappers ---
    ...
    def _process_stick_input_for_mouse(self, x, y, effective_mode=None):
        """Unified mouse processing logic with optional mode override."""
        active_settings = self.get_active_settings()
        mouse_mode = effective_mode or active_settings.get('dualsense_mouse_mode', 'Browser')
        inner_deadzone = active_settings.get('dualsense_inner_deadzone', 0.08)
        outer_deadzone = active_settings.get('dualsense_outer_deadzone', 0.95)
        ...
        # Apply inner deadzone - return zero if below threshold
        if vector < inner_deadzone:
            return 0, 0
        
        # Apply outer deadzone - clamp if above threshold
        if vector > outer_deadzone:
            # Normalize to outer deadzone boundary
            x = (x / vector) * outer_deadzone
            y = (y / vector) * outer_deadzone
            vector = outer_deadzone

        # Scale to 0-1 range between inner and outer deadzones
        deadzone_range = outer_deadzone - inner_deadzone
        if deadzone_range > 0.001:
            scaled_magnitude = (vector - inner_deadzone) / deadzone_range
            scaled_magnitude = min(1.0, max(0.0, scaled_magnitude))  # Clamp to 0-1
        else:
            scaled_magnitude = 1.0  # Fallback if deadzones are too close

        # Apply curve/exponent
        use_exponent = active_settings.get(f'{mouse_mode.lower()}_exponent_enabled', True)
        exponent = active_settings.get(f'{mouse_mode.lower()}_exponent', 2.0)
        
        curved_magnitude = scaled_magnitude
        if use_exponent and exponent > 0:
            curved_magnitude = scaled_magnitude ** exponent

        # Calculate final output
        if vector > 0:
            final_x = (x / vector) * curved_magnitude * sensitivity
            final_y = (y / vector) * curved_magnitude * sensitivity
            ...
            try:
                if self.controller:
                    ...
                    if active_settings.get('dualsense_mouse_enabled'):
                        stick_value = self.controller.right_stick.value
                        rx_raw, ry_raw = stick_value.x, stick_value.y
                        norm_x = (rx_raw - 127.5) / 127.5
                        norm_y = (ry_raw - 127.5) / 127.5
                        
                        # NEW: Get effective mouse mode based on adaptive setting
                        effective_mouse_mode = self._get_effective_mouse_mode(active_settings)
                        
                        # Pass effective mode to processing functions
                    ...
                    # --- Update Key Press Requests ---
                    # Stick-to-Keys
                    if active_settings.get('dualsense_keys_enabled'):
                        stick_value = self.controller.left_stick.value
                        lx_raw, ly_raw = stick_value.x, stick_value.y
    ...
    def populate_dualsense_mouse_tab(self, parent_frame):
        ...
        # --- NEW: Adaptive Game Mode Checkbox ---
        self.adapt_game_var = tk.BooleanVar()
        ttk.Checkbutton(general_frame, text="Adaptive game mode (Requires game mode to be active)", 
                        variable=self.adapt_game_var).grid(row=3, column=0, columnspan=2, sticky='w', pady=5)

        self.sensitivity_var = tk.StringVar()
        self.inner_deadzone_var = tk.StringVar()
        self.outer_deadzone_var = tk.StringVar()
        
        # --- MODIFIED: Grid layout rows adjusted for new checkbox ---
        fields = [("Sensitivity:", self.sensitivity_var), ("Inner Deadzone (0-1):", self.inner_deadzone_var), ("Outer Deadzone (0-1):", self.outer_deadzone_var)]
        for i, (text, var) in enumerate(fields, start=4): # Start from row 4
            ttk.Label(general_frame, text=text).grid(row=i, column=0, sticky='w', padx=5, pady=5)
            ttk.Entry(general_frame, textvariable=var, width=10).grid(row=i, column=1, sticky='w', padx=5, pady=5)
            
        self.invert_x_var = tk.BooleanVar()
        self.invert_y_var = tk.BooleanVar()
        ttk.Checkbutton(general_frame, text="Invert Mouse X-Axis", variable=self.invert_x_var).grid(row=len(fields)+4, column=0, sticky='w', padx=5, pady=5) # Use row + 4
        ttk.Checkbutton(general_frame, text="Invert Mouse Y-Axis", variable=self.invert_y_var).grid(row=len(fields)+4, column=1, sticky='w', padx=5, pady=5) # Use row + 4
        ...
        # NEW: Sensitivity Multiplier for Game Mode
    ...
    def populate_dualsense_keys_tab(self, parent_frame):
        ...
        ttk.Checkbutton(top_frame, text="Enable Left Stick Key Mapping", variable=self.dualsense_keys_enabled_var).pack(side='left', padx=10)
        ...
        bindings_frame_container = ttk.LabelFrame(main_container, text="Left Stick Bindings")
    ...
    def redraw_keys_canvas(self, *args):
        ...
        stick_obj = self.analog_states['left_stick']
        x_val, y_val = stick_obj[0], stick_obj[1]
        x_offset_norm, y_offset_norm = (x_val - 127.5) / 127.5, (y_val - 127.5) / 127.5
        
        self.draw_stick_indicator_on_canvas(self.keys_canvas, center_x, center_y, x_offset_norm, y_offset_norm)

    def draw_stick_indicator_on_canvas(self, canvas, base_x, base_y, x_val, y_val):
        max_offset = 60
        offset_x, offset_y = int(x_val * max_offset), int(y_val * max_offset)
        
        ring_radius = 60
        canvas.create_oval(base_x - ring_radius, base_y - ring_radius, base_x + ring_radius, base_y + ring_radius, outline='#007ACC', width=2)
        
        if abs(x_val) > 0.05 or abs(y_val) > 0.05:
            canvas.create_line(base_x, base_y, base_x + offset_x, base_y + offset_y, fill='#007ACC', width=2)
        
        dot_radius = 8
        canvas.create_oval(base_x + offset_x - dot_radius, base_y + offset_y - dot_radius, base_x + offset_x + dot_radius, base_y + offset_y + dot_radius, fill='#007ACC', outline='white')
    ...
    def populate_compatibility_tab(self, parent_frame):
        ...
        ttk.Radiobutton(mouse_frame, text="pynput (Default, Recommended)", variable=self.mouse_compat_var, value="pynput").pack(anchor='w', padx=10, pady=2)
        ttk.Radiobutton(mouse_frame, text="Win32 API (ctypes, for legacy games)", variable=self.mouse_compat_var, value="ctypes").pack(anchor='w', padx=10, pady=2)
        ...
        ttk.Radiobutton(keyboard_frame, text="pynput (Default, Recommended)", variable=self.keyboard_compat_var, value="pynput").pack(anchor='w', padx=10, pady=2)
        
        direct_input_rb = ttk.Radiobutton(keyboard_frame, text="DirectInput (for DirectX games)", variable=self.keyboard_compat_var, value="pydirectinput")
    ...
    def _save_settings_from_gui(self, win):
        ...
        try:
            ...
            active_settings['dualsense_inner_deadzone'] = float(self.inner_deadzone_var.get())
            active_settings['dualsense_outer_deadzone'] = float(self.outer_deadzone_var.get())
            ...
            # --- FIXED: Corrected saving of the adaptive game mode setting ---
    ...
    def _load_settings_into_ui(self):
        ...
        self.inner_deadzone_var.set(str(active_settings.get('dualsense_inner_deadzone')))
        self.outer_deadzone_var.set(str(active_settings.get('dualsense_outer_deadzone')))
        ...
        # --- FIXED: Corrected typo 'adaptable_game_mode' to 'adaptive_game_mode' to prevent TypeError ---
        ...
        try:
            ...
            while not self.update_queue.empty():
                ...
                if update_type == 'status':
                    ...
                    should_redraw_tester = True
                    if data[0] == 'left_stick': should_redraw_keys = True
    ...
    def draw_analog_sticks(self, draw):
        ...
        for stick_name, base_pos in stick_positions.items():
            ...
            x_val, y_val = stick_obj[0], stick_obj[1]
            x_offset_norm, y_offset_norm = (x_val - 127.5) / 127.5, (y_val - 127.5) / 127.5
            self.draw_stick_indicator(draw, base_pos, x_offset_norm, y_offset_norm)
    ...
    def draw_stick_indicator(self, draw, base_pos, x_val, y_val):
        ...
        if abs(x_val) > 0.05 or abs(y_val) > 0.05:
            draw.line([base_x, base_y, base_x + offset_x, base_y + offset_y], fill=(0, 255, 255, 200), width=int(2 * uniform_scale))
        dot_radius = int(8 * uniform_scale)
        draw.ellipse([base_x + offset_x - dot_radius, base_y + offset_y - dot_radius, base_x + offset_x + dot_radius, base_y + offset_y + dot_radius], fill=(0, 255, 255, 220), outline='white')
    ...
    def create_tray_icon(self):
        try:
            return Image.open(self.resource_path("icon.ico"))
        except Exception:
    ...
    def run(self):
        if not DualSenseController:
            root = tk.Tk()
            root.withdraw()
            messagebox.showerror("Missing Library", "The required library `dualsense-controller` is not installed.\n\nPlease install it using: pip install dualsense-controller")
            return
        ...
        self.icon = Icon("UtilityApp", image, "Controller & Key Utility", menu)
